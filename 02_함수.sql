...
-- 함수는 SELECT문의
-- SELECT 절, WHERE 절, OTHER BY절, GROUP BY절, HAVING절 사용 가능
...
-- 컬럼이나 문자열에서 지정한 위치부터 지장된 길이만큼 문자열을 잘라내서 반환
--> 잘라낼 길이 생략 시 끝까지 잘라냄
...
SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0번 째 자리에서 반올림해서
																		 -- 소수점 -1 자리 표현
...
-- TRUNC(숫자 | 컬럼명 [, 유ㅏ차]) : 특정 위치 아래를 버림(절삭)

SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래 버림
 
SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫 째 자리 아래 버림
...
-- 2024-02-29 11:21:43.524 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)
...
-- ADD_MAONTH(날짜, 숫자) :  날짜에 숫자만큼의 개월 수를 더함(음수도 가능)
...
-- EXTRACT(MONTH FROM 날짜) : 월만 추춘
-- EXTRACT( FROM 날짜) : 일만 추춘

-- EMPLOYEE 테이블에서
-- 각 사원의 일름, 입사년도, 월, 일 조회
SELECT EMP_NAME,
...
-- 년,월,일이 날짜를 나타내는 패턴으로 인식이 안되어서 오류 발생
...
-- 해당 컬럼의 값이 있으면 바꿀 값 1로 변경,
-- 해당 컬럼이 NULL이면 바꿀 값 2로 변경
...
-- 급여 최대값, 가장 늦은 입사일, 알파벳 순서가 가장 느린 이메일을 조회
